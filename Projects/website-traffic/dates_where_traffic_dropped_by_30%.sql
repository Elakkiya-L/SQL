...
select visit_date,total_visitors, lag(total_visitors) over(order by visit_date) as prev_day_visitors
 from daily_traffic)
 select  visit_date,round ((prev_day_visitors-total_visitors)/prev_day_visitors*100 ,2)
 from previous_day_traffic
 where prev_day_visitors>0  and 
 (prev_day_visitors-total_visitors) / prev_day_visitors >= 0.30
 ;
